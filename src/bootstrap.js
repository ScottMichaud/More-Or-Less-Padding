...
function loadMainStyleSheet() {
  ...
  switch (osString) {
    case "WINNT":
    urlString = 'chrome://moreorlesspadding/content/styleWinNone.css';
    break;
    case "Linux":
    urlString = 'chrome://moreorlesspadding/content/styleLinuxNone.css';
    break;
    case "Darwin":
    urlString = 'chrome://moreorlesspadding/content/styleMacNone.css';
    break;
...
function unloadMainStyleSheet() {
  ...
  if(sss.sheetRegistered(srcCSS, sss.USER_SHEET)) {
    sss.unregisterSheet(srcCSS, sss.USER_SHEET);
  }; //Make sure nothing was double-registered.
     //Should only happen in LighterWeight Themes for Australis, but no harm.
};
...
var StyleManager = {
  ...
  setCss: function() {
    ...
    if (osString === "Darwin") {
      wPaddingTop = 10; //Disable top padding for "windowed mode" in Mac OSX. Too buggy.
    };
